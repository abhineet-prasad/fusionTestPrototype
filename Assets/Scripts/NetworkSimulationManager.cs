    ...
    public class NetworkSimulationManager : NetworkBehaviour
    {
        private static NetworkSimulationManager s_Instance;
        protected static NetworkSimulationManager Instance
        {
            get
            {
                if (!s_Initialized)
                {
                    s_Instance = new GameObject("NetworkSimulationManager").AddComponent<NetworkSimulationManager>();
                    s_Initialized = true;
                }
                return s_Instance;
            }
        ...
        Opsive.Shared.Game.Scheduler.ScheduleFixed(Time.fixedDeltaTime / 2, Instance.SetUpdateOrderInternal, firstLocomotion, secondLocomotion);
        }
